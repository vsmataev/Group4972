...
if(InputNum!=null) // знак != означает неравенство
{
    ...
    int? num = int.Parse((InputNum)?? "0"); // Parse - перевод значения в числовую форму
...
Console.WriteLine (($"квадрат числа {InputNum} равен "+Math.Pow(int.Parse(InputNum),2))?? "0"); //через функцию Math.Pow