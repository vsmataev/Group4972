...
//Вводим данные
int ReadData(string msg) //из 17 задачи
{
...
int SimpleSum(int numA)
{
    ...
    {
        sumOfNumber+=i; //эквивалентно sumOfNumber = sumOfNumber + i;
    }
    return sumOfNumber;
}

int GausseSum(int numA) //по формуле Гаусса
{
    int sumOfNumber = 0;
...
//определяет время выполнения (скорость) выполнения
